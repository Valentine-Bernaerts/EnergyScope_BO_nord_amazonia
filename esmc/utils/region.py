...
class Region:
    ...
    def read_tech(self):
        ...
        if r_path.is_file():
            if self.ref_region:
                ...
                df = clean_indices(df)
                if 'Technologies' in self.data and not self.data['Technologies'].empty:
                    for col, dtype in self.data['Technologies'].dtypes.items():
    ...
    def compute_peak_sh_and_sc(self):
        ...
        ts_level0 = self.ts_td.index.get_level_values(0)
        if 'HEAT_LOW_T_SH' in ts_level0:
            ...
            self.peak_sh_factor = 1.0

        if 'SPACE_COOLING' in ts_level0:
