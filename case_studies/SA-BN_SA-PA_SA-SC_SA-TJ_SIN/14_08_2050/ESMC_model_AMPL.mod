...
set GROUPED_REGIONS := {'SIN'};

set TECH_HS := {'PV_HS', 'HS_DIESEL', 'BATT_HS'};
...
subject to extra_grid{c in REGIONS}:
    F [c,"GRID"] = 1 + (c_grid_extra / c_inv[c,"GRID"]) *( (F [c,"WIND_ONSHORE"]     + F [c,"WIND_OFFSHORE"]     + F [c,"PV_ROOFTOP"]   + F [c,"PV_UTILITY"]   + F [c,"PV_HS"]   )
					                                     - (f_min [c,"WIND_ONSHORE"] + f_min [c,"WIND_OFFSHORE"]  + f_min [c,"PV_ROOFTOP"] + f_min [c,"PV_UTILITY"] + f_min [c,"PV_HS"]) );
...
# new eq. Impose batt_hs to be suplied by pv_hs
subject to HS_storage {c in REGIONS, h in HOURS, td in TYPICAL_DAYS}:
	Storage_in [c, "BATT_HS", "ELECTRICITY",h,td] <=  layers_in_out["PV_HS", "ELECTRICITY"]* F_t[c, "PV_HS", h, td];

subject to pv_battery_ratio_hs_min {c in REGIONS}:
	pv_battery_ratio_min * F[c, "BATT_HS"] <= F[c, "PV_HS"];
subject to pv_battery_ratio_hs_max {c in REGIONS}:
