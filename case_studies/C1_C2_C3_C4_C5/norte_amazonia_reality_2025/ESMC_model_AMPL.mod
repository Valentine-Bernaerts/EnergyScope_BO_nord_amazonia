...
set GROUPED_REGIONS := {'SIN'} inter REGIONS;

set TECH_HS := {'PV_HS', 'HS_DIESEL', 'BATT_HS'} inter TECHNOLOGIES;
...
set RELATED_PAIRS within {GROUPED_REGIONS, REGIONS} =
    {g in GROUPED_REGIONS, r in REGIONS : r not in GROUPED_REGIONS};
...
param heating_time_series {REGIONS, HOURS, TYPICAL_DAYS} >= 0, <= 1, default 0; # %_sh [-]: factor for sharing space heating across typical days (adding up to 1)
...
param share_short_haul_flights_max{REGIONS} >= 0, <= 1; # %_aviation,max [-]: max limit for share of short-haul flight over total mobilityparam share_mobility_public_max{REGIONS} >= 0, <= 1; # %_public,max [-]: max limit for penetration of public mobility over total mobility
...
var R_t_local {REGIONS, RESOURCES , HOURS, TYPICAL_DAYS} >= 0; # Use of resource from local feedstock at each period [GW]
var R_t_exterior {REGIONS, RESOURCES , HOURS, TYPICAL_DAYS} >= 0; # Import of resource from the exterior of the overall system at each period [GW]
...
# [Eq. 3] Investment cost of each technology — charge capex only on NEW capacity (brownfield)
subject to investment_cost_calc {c in REGIONS, j in TECHNOLOGIES}:
	C_inv [c,j] = c_inv [c,j] * (F [c,j] - f_min [c,j]);
...
subject to impose_non_daily_storage {c in REGIONS, j in STORAGE_TECH diff STORAGE_DAILY, h in HOURS, td in TYPICAL_DAYS}:
	F_t [c, j, h, td] = 0;
...
subject to limit_energy_to_power_ratio {c in REGIONS, j in STORAGE_TECH diff {"BEV_BATT","PHEV_BATT","SUV_ELEC_PRIVATE_BATT","MOTORCYCLE_ELEC_PRIVATE_BATT","BUS_ELEC_PUBLIC_BATT"}, l in LAYERS, h in HOURS, td in TYPICAL_DAYS}:
...
subject to network_losses {c in REGIONS, eut in END_USES_TYPES, h in HOURS, td in TYPICAL_DAYS}:
	Network_losses [c,eut,h,td] = (sum {j in TECHNOLOGIES diff STORAGE_TECH diff TECH_HS: layers_in_out [j, eut] > 0} ((layers_in_out[j, eut]) * F_t [c, j, h, td])) * loss_network [eut] + (sum {i in RESOURCES} ((layers_in_out[i, eut]) * R_t_exterior[c, i, h, td])) * loss_network [eut];

#subject to network_losses_HS {c in REGIONS, eut in END_USES_TYPES, h in HOURS, td in TYPICAL_DAYS}:
#	Network_losses [c,eut,h,td] = (sum {j in TECH_HS diff STORAGE_TECH: layers_in_out [j, eut] > 0} ((layers_in_out[j, eut]) * F_t [c, j, h, td])) * 0.02;


#subject to share_tech_hs {c in REGIONS, h in HOURS, td in TYPICAL_DAYS}:
#		sum {j in TECH_HS diff STORAGE_TECH: layers_in_out[j, "ELECTRICITY"] > 0} (layers_in_out[j, "ELECTRICITY"] * F_t[c, j, h, td]) + sum {j in TECH_HS inter STORAGE_TECH} Storage_out[c, j, "ELECTRICITY", h, td] >= share_dispersion[c] * sum {j in TECHNOLOGIES diff STORAGE_TECH: layers_in_out[j, "ELECTRICITY"] > 0} (layers_in_out[j, "ELECTRICITY"] * F_t[c, j, h, td]);

subject to share_tech_hs {c in REGIONS, h in HOURS, td in TYPICAL_DAYS}:
#sum {j in TECH_HS diff STORAGE_TECH: layers_in_out[j, "ELECTRICITY"] > 0} (layers_in_out[j, "ELECTRICITY"] * F_t[c, j, h, td]) +     sum {j in TECH_HS inter STORAGE_TECH} (Storage_out[c, j, "ELECTRICITY", h, td] - Storage_in[c, j, "ELECTRICITY", h, td]) >=     share_dispersion[c] * (sum {j in TECHNOLOGIES diff STORAGE_TECH: layers_in_out[j, "ELECTRICITY"] > 0} (layers_in_out[j, "ELECTRICITY"] * F_t[c, j, h, td]) + layers_in_out["ELECTRICITY", "ELECTRICITY"] * (R_t_local[c, "ELECTRICITY", h, td] + R_t_exterior[c, "ELECTRICITY", h, td] + R_t_import[c, "ELECTRICITY", h, td] - R_t_export[c, "ELECTRICITY", h, td]));
	sum {j in TECH_HS diff STORAGE_TECH: layers_in_out[j, "ELECTRICITY"] > 0} (layers_in_out[j, "ELECTRICITY"] * F_t[c, j, h, td]) +     sum {j in TECH_HS inter STORAGE_TECH} (Storage_out[c, j, "ELECTRICITY", h, td] - Storage_in[c, j, "ELECTRICITY", h, td]) >=     share_dispersion[c] * (sum {j in TECHNOLOGIES diff STORAGE_TECH: layers_in_out[j, "ELECTRICITY"] > 0} (layers_in_out[j, "ELECTRICITY"] * F_t[c, j, h, td]) + layers_in_out["ELECTRICITY", "ELECTRICITY"] * (R_t_local[c, "ELECTRICITY", h, td] + R_t_exterior[c, "ELECTRICITY", h, td]));

#subject to share_tech_hs_annual {c in REGIONS}:
#sum {t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} (sum {j in TECHNOLOGIES diff TECH_HS diff STORAGE_TECH}(F_t[c, j, h, td] * t_op[h, td]) + (R_t_local[c, "ELECTRICITY", h, td] + R_t_exterior[c, "ELECTRICITY", h, td] +          R_t_import[c, "ELECTRICITY", h, td] - R_t_export[c, "ELECTRICITY", h, td]) * t_op[h, td]) <= (1 - share_dispersion[c]) * sum {t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} (sum {j in TECHNOLOGIES diff STORAGE_TECH}             (F_t[c, j, h, td] * t_op[h, td]) + (R_t_local[c, "ELECTRICITY", h, td] + R_t_exterior[c, "ELECTRICITY", h, td] + R_t_import[c, "ELECTRICITY", h, td] - R_t_export[c, "ELECTRICITY", h, td]) * t_op[h, td]);
#	sum {t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} (sum {j in TECHNOLOGIES diff TECH_HS diff STORAGE_TECH}(F_t[c, j, h, td] * t_op[h, td]) + (R_t_local[c, "ELECTRICITY", h, td] + R_t_exterior[c, "ELECTRICITY", h, td]) * t_op[h, td]) <= (1 - share_dispersion[c]) * sum {t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} (sum {j in TECHNOLOGIES diff STORAGE_TECH}             (F_t[c, j, h, td] * t_op[h, td]) + (R_t_local[c, "ELECTRICITY", h, td] + R_t_exterior[c, "ELECTRICITY", h, td]) * t_op[h, td]);
...
subject to extra_grid{c in REGIONS: c_inv[c,"GRID"] > 0}:
    F [c,"GRID"] = 1 + (c_grid_extra / c_inv[c,"GRID"]) *( (F [c,"WIND_ONSHORE"]     + F [c,"WIND_OFFSHORE"]     + F [c,"PV_ROOFTOP"]   + F [c,"PV_UTILITY"]   + (if "PV_HS" in TECHNOLOGIES then F [c,"PV_HS"] else 0)   )
					                                     - (f_min [c,"WIND_ONSHORE"] + f_min [c,"WIND_OFFSHORE"]  + f_min [c,"PV_ROOFTOP"] + f_min [c,"PV_UTILITY"] + (if "PV_HS" in TECHNOLOGIES then f_min [c,"PV_HS"] else 0)) );
subject to extra_grid_no_inv{c in REGIONS: c_inv[c,"GRID"] = 0}:
    F [c,"GRID"] = 1;
...
# new eq. Impose batt_hs to be suplied by pv_hs (only when TECH_HS technologies exist in data)
...
# [Eq. 2.19-bis] limit the Energy to power ratio for EVs_BATT_OF_V2G. 
...
# Decentralised heat production (Thermal solar & thermal storage)
#-----------------------------------------------------------------
